...
public class Bailiff extends java.rmi.server.UnicastRemoteObject    // for RMI
    implements BailiffInterface // for clients
{
    ...
    public String ping() throws java.rmi.RemoteException {
        ...
        return String.format("Ping response from Bailiff %s on host %s [%s]", id, myHostName, myInetAddress.getHostAddress());
    }
     ...
     */
    public void migrate(Object obj, String cb, Object[] args) throws java.rmi.RemoteException, NoSuchMethodException {

        log.fine(String.format("migrate obj=%s cb=%s args=%s", obj.toString(), cb, Arrays.toString(args)));
     ...
     */
    public Bailiff(String id, String info, Logger log) throws java.rmi.RemoteException, java.net.UnknownHostException, java.io.IOException {
        ...
        log.info(String.format("STARTING id=%s info=%s host=%s debug=%b", id, info, myHostName, debug));

        // Compose the service name under which to register

        serviceName = getClass().getName() + "." + id + "." + Integer.toString((int) (Math.random() * (float) 0x7FFF_FFFF));
    ...
    public String toString() {
        return String.format("Bailiff %s (%s) on host %s [%s]", id, info, myHostName, myInetAddress.getHostAddress());
    }

    /* ================ m a i n   s u p p o r t ================ */

    private static void showUsage() {
        String[] msg = {"Usage: {'?',-h,-help}|[-id string][-info string][-log n]", "? -h help     This message", "-id   string  Sets the identification string of this Bailiff", "-info string  Sets the information message of this Bailiff", "-log  n       Sets the logging level, higher is more:", "  -log 0        Level.OFF", "  -log 3        Level.INFO", "  -log 7        Level.ALL"};
        for (String s : msg)
            System.out.println(s);
    }
        ...
        for (String av : argv) {
            ...
            switch (state) {
                ...
                case 0:
                    ...
                    else if (av.equals("-info")) state = 2;
                    else if (av.equals("-log")) state = 3;
                    else {
